...
targetInfo::targetInfo(std::string name, int start, int end) {
  target_name = name;
  byte_start = start;
  byte_end = end;
  is_open = false;
...
targetInfo *taintSourceManager::getTargetInfo(std::string name) {
  taint_info_mutex.lock();
  if (name_target_map.find(name) == name_target_map.end()) {
    taint_info_mutex.unlock();
    return nullptr;
...
targetInfo *taintSourceManager::getTargetInfo(int fd) {
  taint_info_mutex.lock();
...
targetInfo *taintSourceManager::getTargetInfo(FILE *fd) {
  taint_info_mutex.lock();
...
targetInfo *taintSourceManager::findTargetInfo(std::string name) {
  taint_info_mutex.lock();
  if (name_target_map.find(name) != name_target_map.end()) {
    taint_info_mutex.unlock();
...
std::map<std::string, targetInfo *> taintSourceManager::getTargets() {
  return name_target_map;
}

json taintSourceManager::getMetadata(targetInfo *targ_info) {
